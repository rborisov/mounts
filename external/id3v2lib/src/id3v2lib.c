...
ID3v2_tag* load_tag_with_buffer(char *bytes, int length)
{
    ...
    version = get_tag_version(tag_header);
    if(version == NO_COMPATIBLE_TAG)
    {
        // no supported id3 tag found
        printf("no supported id3 tag found\n");
        free(tag_header);
        return NULL;
    } else {
        if (version == ID3v23) {
    ...
    if(length < tag_header->tag_size+10)
    {
        ...
        printf("Not enough bytes provided to parse completely.\n");
        free(tag_header);
        return NULL;
    }
